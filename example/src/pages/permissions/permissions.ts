...
import { Capacitor, PermissionType } from "@lambda-capacitor/core";
...
export class PermissionsPage {
  ...
  async checkCamera() {
    const has = await Plugins.Permissions.query({
  ...
  async checkClipboard() {
    const hasRead = await Plugins.Permissions.query({
      name: PermissionType.ClipboardRead
    });
    const hasWrite = await Plugins.Permissions.query({
