...
import { Plugins, PhotosResult } from "@lambda-capacitor/core";
...
export class PhotosPage {
  ...
  async loadInFavorites() {
    ...
    var favorites = result.albums.filter(
      album => album.name.toLowerCase() == "favorites"
    )[0];
  ...
  async savePhoto() {
    ...
    var result = await Plugins.Photos.getAlbums();
    var targetAlbum = result.albums.filter(album => album.type == "user")[0];

    Plugins.Photos.savePhoto({
      data: img,
      albumIdentifier: targetAlbum && targetAlbum.identifier
    });
  }
