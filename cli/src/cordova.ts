...
export async function copyCordovaJS(config: Config, platform: string) {
  ...
  if (!cordovaPath) {
    logFatal(
      `Unable to find node_modules/@lambda-capacitor/core/cordova.js. Are you sure`,
      "@lambda-capacitor/core is installed? This file is currently required for Capacitor to function."
    );
    return;
  }
...
export async function writeCordovaAndroidManifest(
  ...
  let cleartext = config.app.extConfig.server?.cleartext
