...
export function generatePodFile(config: Config, plugins: Plugin[]) {
  const capacitoriOSPath = resolveNode(config, "@lambda-capacitor/ios");
  if (!capacitoriOSPath) {
    logFatal(
      `Unable to find node_modules/@lambda-capacitor/ios. Are you sure`,
      `@lambda-capacitor/ios is installed? This file is currently required for Capacitor to function.`
    );
    return;
  }
  ...
  const relativeCapacitoriOSPath = convertToUnixPath(
    relative(podfilePath, capacitoriOSPath)
  );
