...
export async function createCommand(
  ...
  try {
    ...
    await runTask(chalk`Installing dependencies`, () => {
      return installDeps(
        appDir,
        ["@lambda-capacitor/cli", "@lambda-capacitor/core"],
        config
      );
    });
    ...
    if (typeof e === "string") {
      log(
        "Usage: npx @lambda-capacitor/cli create appDir appName appId npmClient?"
      );
      log(
        'Example: npx @lambda-capacitor/cli create my-app "My App" "com.example.myapp"'
      );
    }
