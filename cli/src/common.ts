...
export async function checkNpmClient(
  config: Config,
  client: string
): Promise<string | null> {
...
export async function checkPlatformVersions(config: Config, platform: string) {
  const cliPackagePath = resolveNode(
    config,
  ...
  if (!cliPackagePath) {
    logFatal(
      "Unable to find node_modules/@lambda-capacitor/cli/package.json. Are you sure",
      "@lambda-capacitor/cli is installed? This file is currently required for Capacitor to function."
    );
    return;
  }
  ...
  if (semver.gt(cliVersion, platformVersion)) {
    log("\n");
    logInfo(
      `Your @lambda-capacitor/cli version is greater than @capacitor/${platform} version`
    );
    log(
      `Consider updating to matching version ${chalk`{bold npm install @capacitor/${platform}@${cliVersion}}`}`
    );
  }
}
